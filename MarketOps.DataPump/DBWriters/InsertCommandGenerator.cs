...
namespace MarketOps.DataPump.DBWriters
{
    ...
    internal class InsertCommandGenerator
    {
        ...
        {
            return $"insert into {_dataPumpProvider.GetTableName(stockDefinition.Type, StockDataRange.Day, 0)}(fk_id_spolki, ts, open, high, low, close, volume) values " +
                $"({stockDefinition.ID}, to_date('{PrepareDate(data.TS)}', 'YYYY-MM-DD'), " +
                $"{PrepareFloat(data.O)}, {PrepareFloat(data.H)}, {PrepareFloat(data.L)}, {PrepareFloat(data.C)}, {data.V})";
        }
    }
}
