...
namespace MarketOps.System.Tests.Extensions
{
    ...
    public class PositionExtensionsTests
    {
        ...
        [TestCase(0, 1, 10, 10)]
        [TestCase(1, 1, 10, 0)]
        public void ClosedValue_Long(float open, float close, int volume, float expected)
        {
            _testObj.Direction = PositionDir.Long;
            _testObj.Open = open;
            _testObj.Close = close;
            _testObj.Volume = volume;
            _testObj.ClosedValue().ShouldBe(expected);
        }
        ...
        [TestCase(0, 1, 10, -10)]
        [TestCase(1, 1, 10, 0)]
        public void ClosedValue_Short(float open, float close, int volume, float expected)
        {
