...
{
    /// <summary>
    /// Simplex stocks data calculator.
    /// </summary>
    internal static class SimplexStocksDataCalculator
    {
        public static void Initialize(SimplexStocksData data, string[] fundsNames, IStockDataProvider dataProvider)
        {
            for (int i = 0; i < fundsNames.Length; i++)
                data.Stocks[i] = dataProvider.GetStockDefinition(fundsNames[i]);
        }

        public static void Calculate(SimplexStocksData data, DateTime ts, int profitRange, int changeRange, StockDataRange dataRange, ISystemDataLoader dataLoader)
        {
