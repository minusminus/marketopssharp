...
namespace MarketOps.Tests.Stats.Calculators
{
    ...
    public class RangeChangePcntTests
    {
        ...
        {
            RangeChangePcnt.Calculate(data, range).ShouldBe(expected, 0.0001f);
        }
        ...
        {
            RangeChangePcnt.Calculate(data, 4).ShouldBeEmpty();
        }
        ...
        {
            RangeChangePcnt.Calculate(new float[] { 1, 2, 3, 4 }, range).ShouldBeEmpty();
        }
    }
}
