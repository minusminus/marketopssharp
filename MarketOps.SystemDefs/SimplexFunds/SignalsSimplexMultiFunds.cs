...
namespace MarketOps.SystemDefs.SimplexFunds
{
    ...
    internal class SignalsSimplexMultiFunds : ISystemDataDefinitionProvider, ISignalGeneratorOnClose
    {
        ...
        private const int TruncateBalanceToNthPlace = 3;    //balans in range <0..1> - truncate to first place after comma
        ...
        public List<Signal> GenerateOnClose(DateTime ts, int leadingIndex, SystemState systemState)
        {
            ...
            float[] balance = SimplexExecutor.Execute(_fundsNames, _fundsData,
                portfolioValue, AcceptableSingleDD, RiskSigmaMultiplier, MaxSinglePositionSize, MaxPortfolioRisk, TruncateBalanceToNthPlace);
